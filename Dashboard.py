...
#RRENDIMIENTO DE TODOS LOS FONDOS CON TODAS LAS SERIES 
#fondos_rend = fondos.pivot(index='FECHA', columns=['EMISORA','INSTRUMENTO','SERIE'], values='PRECIOSUCIO')
#fondos_rend=fondos_rend.pct_change()

#RENDIMIENTO DE TODOS LOS FONDOS CON SERIES B, B1, B-1
fondos_rendb=fondos_b.pct_change()

#RENDIMIENTOS DE TODOS LOS FONDOS ACTINVER CON TODAS LAS SERIES
#fondos_rendact=fondos_actinver.pivot(index='FECHA', columns=['EMISORA','INSTRUMENTO','SERIE'], values='PRECIOSUCIO')
#fondos_rendact=fondos_rendact.pct_change()
#fondos_rendact.columns=fondos_rendact.columns.droplevel(['SERIE','INSTRUMENTO'])
#fondos_rendact.index = pd.to_datetime(fondos_rendact.index)
...
# Sort by descending order of returns
...
def update_graphs(n_clicks, top_n_clicks, start_date, end_date, top):
    ...
    # Sort total returns for each asset
